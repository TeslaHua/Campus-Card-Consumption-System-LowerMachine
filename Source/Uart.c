...
void UART_Init(UARTx uartx)
{
  switch(uartx)
  {
      ...
      case UART1:{//波特率为9600
        ...
        UTCTL1 |= SSEL1;                          // UCLK = SMCLK（外部8M）
        UBR01 = 0x41;                             // 8M/9600 -833.3
        UBR11 = 0x03;                             //
...
返回值  ：无
********************************************/
void Send1Char(UARTx uartx ,uint8 sendchar)
{
      switch(uartx)
      {
        case UART0:{//波特率为9600
...
返回值  ：无
********************************************/
void PutString(UARTx uartx,uint8 *ptr)
{
      switch(uartx)
     {
          case UART0:{//波特率为9600
 ...
 *    串口1接收的字节并发送
 */
void UART1_GetCharINT(void)
{  
     if(RXBUF1=='A'){  //上位机发送来的是报警标志
         for(int i=0;i<20;i++)  //蜂鸣器开始报警
         {
             BuzzerOn1s();  
             Delay(20);
             P1OUT &=~(BIT4);
         ...
         ReceiveBalance[index]=RXBUF1; //收到的余额信息存放处
         index++;
     }
}  
/*******************************************
 ...
 *    串口0接收的字节并发送
 */
void UART0_GetCharINT(void)
{
     Send1Char(UART0,RXBUF0);
}
/*
 * UART_RIE_Enable
 * 使能串口数据接收中断，设置中断函数入口
