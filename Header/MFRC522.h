...
#define OK  1
#define ERROR  0
extern unsigned char Read_Buffer[16];  //读取IC卡的一块数据
extern unsigned char Password_Buffer[6]; // 出厂默认密码
...
//                                                                           //
//***************************************************************************//
#define RF_LPCTL               BIT4  // P2.4 射频卡休眠控制 ------RST   
#define RF_SS                  BIT3  // p2.3 射频卡从机选择(SS)---SDA
#define RF_SCLK                BIT7  // p1.7 射频卡数据时钟输出(SCLK)
#define RF_DATA_OUT            BIT1  // p2.1 射频卡数据输出(MOSI)
#define RF_DATA_IN             BIT0  // p2.0 射频模块输入(MISO) 

//#define RF_LPCTL               BIT3  // P2.4 射频卡休眠控制 ------RST   
//#define RF_SS                  BIT7  // p2.3 射频卡从机选择(SS)---SDA
//#define RF_SCLK                BIT6  // p2.7 射频卡数据时钟输出(SCLK)
//#define RF_DATA_OUT            BIT5  // p2.1 射频卡数据输出(MOSI)
//#define RF_DATA_IN             BIT0  // p2.0 射频模块输入(MISO) 
//****************************************************************************
//                                                                           *
//                           RC522操作宏定义                                      *
...
#define    SCLK_HIGH               P1OUT |= RF_SCLK                      // 数据时钟置高
#define    SCLK_LOW                P1OUT &=~RF_SCLK                      // 数据时钟置低
...
#define    SCK522_1                P1OUT |= RF_SCLK                      // 数据时钟置高
#define    SCK522_0                P1OUT &=~RF_SCLK                      // 数据时钟置低
#define    SI522_1                 P2OUT |= RF_DATA_OUT                  // 数据置高        
#define    SI522_0                 P2OUT &=~RF_DATA_OUT                  // 数据置低        
#define    SO522                   P2IN  &  RF_DATA_IN           // 数据输入
...
//                         函数定义                                 /
//******************************************************************/
void Init_Port(void);  //对应端口定义
void Delay(uint16 time);//软件延时
char PcdHalt(void);   //卡片进入休眠状态
char PcdReset(void);  //复位RC522
void PcdAntennaOn(void);  //开天线接收
void PcdAntennaOff(void); //关天线接收
...
char PcdRead(unsigned char addr,unsigned char *pData); //读取一块数据    
char PcdWrite(unsigned char addr,unsigned char *pData); //写入一块数据   
//扣款和充值
...
void CalulateCRC(unsigned char *pIndata,unsigned char len,
                 unsigned char *pOutData);//用MF522计算CRC16函数
...
void ClearBitMask(unsigned char reg,unsigned char mask);//清除RC522寄存器位 

uint8 EqualBuffer(uint8 Read_Buffer[],uint8 HaltBuffer[]);
unsigned char Auto_Reader(void);//自动寻卡步骤
#endif